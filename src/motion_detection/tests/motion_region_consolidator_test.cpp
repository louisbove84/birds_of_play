...
std::vector<TrackedObject> processRealImages(const std::string& configPath,
    ...
    LOG_INFO("Loaded test images: {}x{} and {}x{}", frame1.cols, frame1.rows, frame2.cols,
             frame2.rows);
    ...
    LOG_INFO("First frame processed - hasMotion: {}, bounds: {}", result1.hasMotion,
             result1.detectedBounds.size());
    ...
    LOG_INFO("Second frame processed - hasMotion: {}, bounds: {}", result2.hasMotion,
             result2.detectedBounds.size());
    ...
    for (const auto& bounds : result2.detectedBounds) {
        ...
        LOG_INFO("Created TrackedObject {}: BBox({},{},{},{}) from real motion detection", objectId,
...
class MotionRegionConsolidatorTest : public ::testing::Test {
   protected:
    void SetUp() override {
        ...
        config.eps = 50.0;
        config.minPts = 2;
        config.overlapWeight = 0.7;
        config.edgeWeight = 0.3;
        config.maxEdgeDistance = 100.0;
        config.regionExpansionFactor = 1.1;
        ...
        LOG_INFO("SetUp complete: {} real TrackedObjects available for testing",
                 realTrackedObjects.size());
    }
...
TEST_F(MotionRegionConsolidatorTest, RealBirdImageConsolidation) {
    ...
    LOG_INFO("Testing consolidation with {} real TrackedObjects from bird images",
             realTrackedObjects.size());

    // Create a more permissive config for bird motion data
    ConsolidationConfig birdConfig = config;
    birdConfig.eps = 200.0;  // Birds can be further apart
    birdConfig.minPts = 1;   // Allow single-bird regions for analysis
    ...
    std::string outputPath =
        "test_results/motion_region_consolidator/google_test_mode/"
        "01_real_bird_consolidation_visualization.jpg";
    auto regions = birdConsolidator.consolidateRegionsWithVisualization(realTrackedObjects,
                                                                        inputImage, outputPath);
    ...
    for (size_t i = 0; i < regions.size(); ++i) {
        ...
        LOG_INFO("Region {}: BBox({},{},{},{}) contains {} objects", i, region.boundingBox.x,
                 region.boundingBox.y, region.boundingBox.width, region.boundingBox.height,
                 region.trackedObjectIds.size());
    }

    // Validate that consolidation produces reasonable results
    EXPECT_LE(regions.size(), realTrackedObjects.size())
        << "Should not create more regions than input objects";
    ...
    for (const auto& region : regions) {
        EXPECT_GE(region.trackedObjectIds.size(), birdConfig.minPts)
            << "Each region should meet minimum object count";
        EXPECT_GT(region.boundingBox.area(), 0) << "Each region should have positive area";
        // With DBSCAN, we don't enforce size constraints, just verify regions are reasonable
        EXPECT_GT(region.boundingBox.width, 0) << "Region should have positive width";
        EXPECT_GT(region.boundingBox.height, 0) << "Region should have positive height";
    }

    // Test with default (stricter) config too
    auto strictRegions = consolidator->consolidateRegions(realTrackedObjects);
    LOG_INFO("Strict config results: {} regions (vs {} permissive)", strictRegions.size(),
             regions.size());
}
...
TEST_F(MotionRegionConsolidatorTest, StandaloneConsolidationSynthetic) {
    ...
    std::string outputPath =
        "test_results/motion_region_consolidator/google_test_mode/02_synthetic_consolidation.jpg";
...
TEST_F(MotionRegionConsolidatorTest, StandaloneConsolidationBirdLike) {
    ...
    // Test standalone consolidation
    std::string outputPath =
        "test_results/motion_region_consolidator/google_test_mode/03_bird_like_consolidation.jpg";
...
TEST_F(MotionRegionConsolidatorTest, EdgeCases) {
    ...
    auto singleRegions = consolidator->consolidateRegionsStandalone(
        singleObject,
        "test_results/motion_region_consolidator/google_test_mode/07_single_object.jpg");
    EXPECT_EQ(singleRegions.size(), 0) << "Single object should not create regions (below minimum)";
    ...
    auto minRegions = consolidator->consolidateRegionsStandalone(
        minObjects, "test_results/motion_region_consolidator/google_test_mode/08_min_objects.jpg");
    EXPECT_GT(minRegions.size(), 0) << "Minimum objects should create at least one region";
}
...
TEST_F(MotionRegionConsolidatorTest, ConsolidationSmallN) {
    ...
    for (int i = 0; i < 10; ++i) {
        objects.emplace_back(i, cv::Rect(100 + i * 10, 100 + i * 10, 20, 20),
                             "uuid" + std::to_string(i));
    }
    for (int i = 10; i < 20; ++i) {
        objects.emplace_back(i, cv::Rect(1000 + i * 10, 1000 + i * 10, 20, 20),
                             "uuid" + std::to_string(i));
    }
    ...
    for (const auto& region : regions) {
        if (region.trackedObjectIds.size() >= static_cast<size_t>(config.minPts)) {
...
TEST_F(MotionRegionConsolidatorTest, ConsolidationLargeN) {
    ...
    for (int i = 0; i < 30; ++i) {
        objects.emplace_back(i, cv::Rect(100 + i * 5, 100 + i * 5, 20, 20),
                             "uuid" + std::to_string(i));
    }
    for (int i = 30; i < 60; ++i) {
        objects.emplace_back(i, cv::Rect(500 + (i - 30) * 100, 500 + (i - 30) * 100, 20, 20),
                             "uuid" + std::to_string(i));
    }
...
TEST_F(MotionRegionConsolidatorTest, ConsolidateRegionsValidInput) {
    ...
    if (!realTrackedObjects.empty() && realTrackedObjects.size() >= 2) {
        ...
        objects.assign(realTrackedObjects.begin(),
                       realTrackedObjects.begin() +
                           std::min(static_cast<size_t>(3), realTrackedObjects.size()));
    } else {
    ...
    LOG_INFO("ConsolidateRegionsValidInput: {} objects -> {} regions", objects.size(),
             regions.size());
    ...
    if (!realTrackedObjects.empty()) {
        ...
        EXPECT_EQ(regions.size(), 1)
            << "Expected one consolidated region for synthetic close objects";
        if (!regions.empty()) {
...
TEST_F(MotionRegionConsolidatorTest, ConsolidateRegionsSingleObject) {
    ...
    EXPECT_TRUE(regions.empty())
        << "Expected no regions for single object (below minObjectsPerRegion)";
}
