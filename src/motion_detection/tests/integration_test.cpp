   ...
   protected:
    void SetUp() override {
        ...
        fs::create_directories(
            "/Users/beuxb/Desktop/Projects/birds_of_play/src/motion_detection/tests/test_results/"
            "integration_test/1_motion_processor_visualizations");
        fs::create_directories(
            "/Users/beuxb/Desktop/Projects/birds_of_play/src/motion_detection/tests/test_results/"
            "integration_test/2_consolidation_visualizations");

        // Initialize motion processor with test config
        motionProcessor = std::make_unique<MotionProcessor>("config.yaml");

        // Initialize consolidation config for DBSCAN
    ...
    void initLogger() {
        try {
            Logger::init("debug",
                         "/Users/beuxb/Desktop/Projects/birds_of_play/src/motion_detection/tests/"
                         "test_results/integration_test/integration_test.log",
                         true);
            ...
            for (const auto& entry : fs::recursive_directory_iterator(baseDir)) {
                if (entry.is_regular_file()) {
                    ...
                    if (ext == ".jpg" || ext == ".png" || ext == ".jpeg") {
                        ...
                        testImages.push_back({entry.path().string(), cleanName});
                    }
                }
            }
        }
    ...
    void processImagePairWithBothComponents(const std::string& image1Path,
        ...
        std::string motionProcessorOutput =
            "/Users/beuxb/Desktop/Projects/birds_of_play/src/motion_detection/tests/test_results/"
            "integration_test/1_motion_processor_visualizations/" +
            pairName + "_motion_processing.jpg";
        std::string consolidationOutput =
            "/Users/beuxb/Desktop/Projects/birds_of_play/src/motion_detection/tests/test_results/"
            "integration_test/2_consolidation_visualizations/" +
            pairName + "_consolidation.jpg";

        auto [result, regions] = processFrameAndConsolidate(*motionProcessor, *regionConsolidator,
                                                            frame2, consolidationOutput);

        // Save MotionProcessor visualization separately
        motionProcessor->saveProcessingVisualization(result, motionProcessorOutput);

        LOG_INFO("Completed {}: {} motion objects -> {} consolidated regions", pairName,
                 result.detectedBounds.size(), regions.size());
    }
...
TEST_F(IntegrationTest, ProcessAllImagePairsWithFullPipeline) {
    ...
    LOG_INFO("Processing {} test images with both MotionProcessor and MotionRegionConsolidator",
             testImages.size());
    ...
    for (const auto& [dir, images] : imagesByDir) {
        if (images.size() >= 2) {
            ...
            for (size_t i = 0; i < images.size() - 1; ++i) {
                ...
                std::string pairName =
                    dir + "_pair_" + std::to_string(i) + "_" + std::to_string(i + 1);
...
TEST_F(IntegrationTest, QuickFileCreationValidation) {
    ...
    for (const auto& [dir, images] : imagesByDir) {
        if (images.size() >= 2 && processedCount < maxToProcess) {
            ...
            std::string motionOutput =
                "/Users/beuxb/Desktop/Projects/birds_of_play/src/motion_detection/tests/"
                "test_results/integration_test/1_motion_processor_visualizations/" +
                pairName + "_motion_processing.jpg";
            std::string consolidationOutput =
                "/Users/beuxb/Desktop/Projects/birds_of_play/src/motion_detection/tests/"
                "test_results/integration_test/2_consolidation_visualizations/" +
                pairName + "_consolidation.jpg";

            EXPECT_TRUE(fs::exists(motionOutput))
                << "Motion processor visualization missing: " << motionOutput;
            EXPECT_TRUE(fs::exists(consolidationOutput))
                << "Consolidation visualization missing: " << consolidationOutput;
